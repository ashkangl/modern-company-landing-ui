...
import { AnimatedButton } from "./AnimatedHamburgerButton";
...
const Header = ({logo = "YOURCOMPANY", ctaText = "Let's Talk"}) => {
    ...
    return (
        <>
        <header className="fixed left-0 top-0 z-50 w-full px-4 pt-4 pb-2 md:px-8 backdrop-blur-md">
            <div className="mx-auto flex max-w-[1600px] items-center justify-between rounded-full border border-black/10 bg-white/80 px-5 py-3 backdrop-blur-xl">
                ...
                <nav className="hidden items-center gap-8 md:flex">
                    {navItems.map((item)=>(
                    <a href={item.path} className="group relative h-6 overflow-hidden text-sm font-medium" key={item.path}>
